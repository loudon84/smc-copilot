...
updated_at: 2026-09-10T05:18:53.610978Z
...
| RM-02 | Enterprise Build Profile & Package Proof：把完整企业 descriptor 确定性注入 Electron package 并验证安装包资源。架构出处：AD-WORK-v4.1.1-SKILL-REGISTRY-ENDPOINT-CONFIGURATION。 | RM-01 | READY | 企业 profile 提供完整 endpoint 集合并通过 preflight；构建产物包含匹配 descriptor；Community 包保持公共默认；企业 profile 缺字段时构建失败；产物与日志无 secret；Windows package evidence 通过。 | - | - | - | - |
