...
updated_at: 2026-09-10T09:35:00+08:00
...
本 Roadmap 从旧的叙述式 milestone 文档迁移为可校验的 Item DAG。仓库尚无覆盖整份 Skill Run 的独立 Architecture Decision，因此 frontmatter `architecture_decision` 仍以已批准父 PRD 的 Target Architecture 作为 provenance bridge；不得把此桥接解释为 M0–M4 已具备新治理所要求的一对一 Stage PRD、Plan、实现提交和验证证据。
...
| RM-16 | M6j Skill Session UX & Terminal Result Closure：修复 Session Files 自动展开、已启动会话技能仍可变更，以及 Provider 已成功但终态报告未进入现有 Skill Card 的一致性缺陷。 | RM-14, RM-15 | DONE | 新会话 Session Files 默认折叠且只由用户显式打开文件/面板触发展示；首个 accepted Skill Run 后会话技能由 Main 权威锁定；SSE/poll 任意先后均保留流式文本并从 v1.5.0 `/result` 收敛终态报告，不重复创建 Run、不暴露 raw Provider data。 | docs/work/PRD-WORK-v4.0.1-M6j-skill-session-ux-terminal-result-closure.md | .cursor/plans/work-v4.0.1-m6j-skill-session-ux-terminal-result-closure.plan.md | 09efa7ac5a539975896b7e9c41cc7397101f36f5 | external-artifact:docs_agent/evidence/RM-17-evidence.json |
| RM-17 | M6j Evidence Closure：不重写 RM-16 实现；以已提交实现 `09efa7ac` 为 grounding baseline，完成 governed completion audit、implementation review、focused verification、durable evidence manifest，并把 RM-16 从 IN_PRD 收口为 DONE。 | - | DONE | RM-16 completion audit PASS；implementation review PASS；V01–V07/V09 fresh PASS；package-wide typecheck 不作为本 Item 阻断项并登记为独立 baseline restoration；durable evidence manifest 存在；RM-16 row DONE（Plan=RM-16 canonical Plan、Commit=09efa7ac、Evidence=external-artifact:docs_agent/evidence/RM-17-evidence.json）；RM-17 row DONE（Plan=RM-17 canonical Plan、Commit=RM-17 implementation commit、Evidence=smc-evidence:RM-17@sha256:<scope-fingerprint>）。 | docs/work/PRD-WORK-v4.0.1-M6j-evidence-closure.md | .cursor/plans/work-v4.0.1-m6j-evidence-closure.plan.md | abc2b8d191db18f4641a3eb31afbf24af39227db | smc-evidence:RM-17@sha256:d6fce26783589898d1b992bc6938f0fd4a583ccf82214a2cd8e21748d6b13156 |
| RM-18 | Work package-wide typecheck baseline restoration：使 `npm --prefix apps/work run typecheck`（node+web）退出码 0。关闭 RM-17 / Managed Hermes RM-02 延后的 BASELINE_RESTORATION_INPUT；仅类型与死代码清理，不改产品行为。 | - | DONE | `typecheck:node` 与 `typecheck:web` 均 PASS；无新生产 Owner；无 Provider/Bundle 变更；focused guard + lat check PASS；durable evidence 证明 typecheck 绿色。 | docs/work/PRD-WORK-v4.0.1-typecheck-baseline-restoration.md | .cursor/plans/work-v4.0.1-typecheck-baseline-restoration.plan.md | dcb13d2980df2c9e611e9ed4f42d154d8bf7e519 | smc-evidence:RM-18@sha256:c2d1dce9b4ec6ea8bee580acf32b009179727ed28dd2a018ea56cecc0f69c4b6 |
...
M0 Provider Contract Ready
  ...
  → M6h Streaming delta 映射（RM-14 DONE）
  → M6i / M6j / Evidence Closure（RM-15–RM-17 DONE）
  → RM-18 package-wide typecheck baseline restoration（READY；关闭 RM-17/RM-02 延后债务）
```
...
**RM-09–RM-12** 各自独立 Grounding。RM-09 / RM-11 已随对应 Bundle 关闭并 DONE。

**RM-13 / M6g — P1 Streaming delta 合同进口**
...
**RM-14 / M6h — P1 Streaming delta 映射**

**Owner:** 现有 Main contract parser + SkillRunService projection；Renderer `modules/skill-run` 只展示已枚举、已清洗的 payload 字段。

**Dependencies:** RM-13 DONE。架构出处：`AD-WORK-v4.0.1-STREAMING-DELTA`。v1.5.0 Bundle 已 checksum lock，本 Item 已 READY，可 Grounding Stage PRD。

**Deliverables:** 仅映射 Bundle 已枚举的 delta 事件与已发布 payload 字段；unknown fail-soft；不新增 Chat 页面、IPC channel 或 Production Owner；不向 Renderer 传递 raw Provider event；不复用 Local Chat 流式通道；不得与 `approvalExpiry`、download-by-ref、clarify respond 合并。「增量文本 / token」在 Provider 枚举 payload 前不是合同事实。
...
- Streaming delta 映射（RM-14）在独立 Item 实施，不得与进口、expiry、upload 混项。
...
- RM-13 / RM-14 是 streaming delta named increment，初始 BACKLOG，不纳入上述 P0/P1 收尾；Provider tag 前不得 READY，不得从父 P1 直接开单一映射 PRD；
...
- P1 与 Expert Removal 各留在独立 Item / PRD / Plan（RM-07–RM-12），不混入 P0 收尾；streaming delta 分 RM-13 进口与 RM-14 映射，均引用 `AD-WORK-v4.0.1-STREAMING-DELTA`。
