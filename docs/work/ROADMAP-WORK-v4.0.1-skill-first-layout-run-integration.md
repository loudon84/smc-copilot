...
| RM-18 | Work package-wide typecheck baseline restoration：使 `npm --prefix apps/work run typecheck`（node+web）退出码 0。关闭 RM-17 / Managed Hermes RM-02 延后的 BASELINE_RESTORATION_INPUT；仅类型与死代码清理，不改产品行为。 | - | READY | `typecheck:node` 与 `typecheck:web` 均 PASS；无新生产 Owner；无 Provider/Bundle 变更；focused guard + lat check PASS；durable evidence 证明 typecheck 绿色。 | - | - | - | - |
