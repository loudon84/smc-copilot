...
| `window.chatRuntime` | `src/preload/chat-runtime-api.ts` | **v8.0** runId 隔离 Chat Runtime（`chat-runtime:submit/abort` + `chat-runtime:event`） |
| `window.chatFiles` | `src/preload/chat-files-api.ts` | **v8.0** Chat Files 薄桥（upload/preview/reveal；不扩展 `hermesAPI.files`） |
...
**v8.0（Copilot Chat Module）：** 默认入口 `HermesDefaultChatPage` → `AiosCopilotChatHost` → `modules/chat` `ChatSurface`；Runtime 走 `window.chatRuntime`（runId 隔离）；Files 走 `window.chatFiles`；`VITE_CHAT_ENGINE=legacy` 回退 `HermesDefaultWebChatSurface.legacy`。
...
| **v8.0** | **Copilot Chat Module 迁移**：runId Chat Runtime IPC、`modules/chat` Surface + AI-OS adapters/slots、`window.chatFiles` 薄桥、File Platform shared 契约、`VITE_CHAT_ENGINE` 切换 | `prd_work/v8.0_upgrade-chat-module.md`, `src/main/chat-runtime/`, `src/shared/chat-runtime/`, `src/renderer/src/modules/chat/`, `scripts/chat-migration/` |
