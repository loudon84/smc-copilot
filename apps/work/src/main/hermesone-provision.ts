// @lat: [[hermes-account-login#SMC Copilot account login#Auto-provisioned inference key and credits]]
...
/**
 * Convenience layer over the signed-in SMC Copilot account: users get model
 * access through SMC Copilot Inference without hand-copying an API key from the
 ...
 * `HERMESONE_API_KEY` — which by itself makes the SMC Copilot provider card and
...
async function doEnsure(profile?: string): Promise<EnsureHermesOneKeyResult> {
  ...
  try {
    ...
    return {
      status: "error",
      error: `Couldn't reach the SMC Copilot backend: ${(err as Error).message}`,
    };
  }
  if (!res.ok) {
    return {
