 ...
 * `stopSshTunnel`, so a 9119�?642 flip-flop yields "SSH tunnel is not active").
 */
...
export function registerIpcHandlers(context: IpcContext): void {
  ...
  // Hermes engine info — local path owned by Copilot Runtime Service
  ...
  // The account is device-wide (one SMC Copilot login for the whole app), but
  ...
  // Auto-provision a SMC Copilot Inference key from the signed-in account when
  ...
  // Cloud agent sync �?reconciles local profiles with the signed-in SMC Copilot
  ...
  ipcMain.handle("list-configured-models", (_event, profile?: string) => {
    const active = profile?.trim() || getActiveProfileNameSync();
    const toConfigured = (mc: {
      provider: string;
      model: string;
      baseUrl: string;
    }): SavedModel[] => {
      ...
      return [
        {
          id: `configured:${mc.provider}\0${mc.model}\0${mc.baseUrl}`,
          name: mc.model.split("/").pop() || mc.model,
          provider: mc.provider || "auto",
          model: mc.model,
          baseUrl: mc.baseUrl || "",
          createdAt: 0,
        },
    ...
    const conn = getConnectionConfig();
    // Remote → local gateway (common desktop.json setup): the agent config still
    // lives on this machine, so read local config.yaml custom_providers instead
    // of only the gateway's currently-active model.
    if (conn.mode === "remote" && isLoopbackUrl(conn.remoteUrl)) {
      return listConfiguredAgentModels(active);
    }
    if (conn.mode === "remote") {
      if (conn.remoteChatTransport === "legacy") {
        throw new Error(
          "Remote configured-model reads require dashboard transport.",
        );
      }
      // True remote host: expose only the dashboard's active model (no remote
      // custom_providers parse yet).
      return withRemoteDashboard(
        conn,
        async () => toConfigured(await remoteGetModelConfig(conn)),
        () => listConfiguredAgentModels(active),
      );
    }
    if (conn.mode === "ssh" && conn.ssh) {
      return withSshDashboardModelLibrary(
        conn,
        async (config) => toConfigured(await remoteGetModelConfig(config)),
        async () =>
          toConfigured(await sshGetModelConfig(conn.ssh!, active)),
        active,
      );
    }
    return listConfiguredAgentModels(active);
