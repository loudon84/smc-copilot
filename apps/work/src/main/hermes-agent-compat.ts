...
# --- HERMES_ONE_MODEL_LIBRARY_COMPAT_V1 -------------------------------------
# Compatibility endpoint installed by SMC Copilot. Upstream Hermes Agent exposes
# /api/model/options and /api/model/set, but SMC Copilot also needs a small
...
export function patchDashboardModelLibrarySource(
  ...
  if (patched === source) {
    return {
      ...
      changed: false,
      source,
      detail: "SMC Copilot model library endpoint is already installed.",
    };
  }
  ...
  return {
    ...
    detail: withoutExisting.removed
      ? "Moved SMC Copilot model library endpoint before the dashboard catch-all route."
      : "Installed SMC Copilot model library endpoint.",
  };
}
    ...
    if '@app.post("/api/model/set")' in source_without_model_library:
        ...
        if source != original_source:
            ...
            if removed_model_library:
                details.append("Moved SMC Copilot model library endpoint before the dashboard catch-all route.")
            else:
                details.append("Installed SMC Copilot model library endpoint.")
        else:
            details.append("SMC Copilot model library endpoint is already installed.")
    else:
        compatible = False
...
export function remoteHttpCompatibilityResult(): HermesAgentCompatResult {
  return {
    ...
    version: HERMES_AGENT_COMPAT_VERSION,
    detail:
      "Plain remote HTTP can be probed but not patched by SMC Copilot. Use SSH mode for deployable compatibility fixes or update the remote Hermes Agent directly.",
  };
}
