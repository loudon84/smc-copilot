...
export function getModelConfig(profile?: string): {
  ...
  // Some agent configs (and hand-edits) put provider/default at the document
  // root instead of under `model:`. Accept that shape so the chat picker and
  // readiness checks still see the active model.
  if (!result.model || result.provider === defaults.provider) {
    const topProvider = content.match(/^provider\s*:\s*["']?([^"'\n#]+)["']?/m);
    const topDefault = content.match(/^default\s*:\s*["']?([^"'\n#]+)["']?/m);
    const topBase = content.match(/^base_url\s*:\s*["']?([^"'\n#]+)["']?/m);
    if (!result.model && topDefault) result.model = topDefault[1].trim();
    if (result.provider === defaults.provider && topProvider) {
      result.provider = topProvider[1].trim();
    }
    if (!result.baseUrl && topBase) result.baseUrl = topBase[1].trim();
  }
