...
interface CustomProviderEntry {
  ...
  apiKey?: string;
  apiMode?: string;
  /** Env var name from `key_env:` when the entry stores the secret outside api_key. */
  keyEnv?: string;
  /** Extra model ids from the nested `models:` map (in addition to `model:`). */
  extraModels?: string[];
}

/**
 * Parse `custom_providers:` from the profile config.yaml. Exported so the
 * Providers screen can import the same list into `providers.json`.
 */
export function loadCustomProviders(profile?: string): CustomProviderEntry[] {
  ...
  const finishCurrent = (): void => {
    if (current && current.baseUrl && (current.model || current.extraModels?.length)) {
      if (!current.model && current.extraModels?.length) {
        current.model = current.extraModels[0];
      }
      result.push(current);
    }
    current = null;
    ...
    if (inCustom) {
      if (/^\s*-\s*name\s*:/.test(line)) {
        ...
        if (/^\s{2,}models\s*:/.test(line)) {
          ...
          const modelId = line.match(/^\s{4,}([A-Za-z0-9._:/-]+)\s*:\s*(?:#.*)?$/);
...
export function listConfiguredAgentModels(profile?: string): SavedModel[] {
  ...
  const keyOf = (provider: string, model: string, baseUrl: string): string =>
    `${provider}\0${model}\0${norm(baseUrl)}`;
  ...
  const push = (entry: {
    ...
    if (!model) return;
    const provider = (entry.provider || "auto").trim() || "auto";
    const baseUrl = (entry.baseUrl || "").trim();
    const key = keyOf(provider, model, baseUrl);
  ...
  try {
    custom = loadCustomProviders(profile);
  } catch (e) {
    console.error("Failed to load custom providers for configured models:", e);
    custom = [];
  }
  for (const cp of custom) {
    const modelIds = Array.from(
      new Set(
        [cp.model, ...(cp.extraModels || [])]
          .map((m) => (m || "").trim())
          .filter(Boolean),
      ),
    );
