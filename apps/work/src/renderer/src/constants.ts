...
export const PROVIDERS = {
  ...
  setup: [
    {
      // SMC Copilot's own inference gateway — shown first. OpenAI-compatible, so
      ...
      id: "hermesone",
      name: "SMC Copilot",
      desc: "SMC Copilot Inference — pay-per-token with AI Credits",
      tag: "Recommended",
 ...
 *
 * OpenAI-compatible providers (SMC Copilot, Groq, DeepSeek, …) are persisted as
 ...
 * under "SMC Copilot" instead of the generic "OpenAI Compatible / Local" bucket.
 *
...
// names. This front-loads the well-known providers — SMC Copilot first — and
...
/**
 * The plain provider name for an LLM-provider env key — "SMC Copilot", not
 * "SMC Copilot API Key". The provider cards/picker are a list of providers, so
...
export const SETTINGS_SECTIONS: SectionDef[] = [
  {
    ...
    items: [
      // SMC Copilot's own inference gateway — first-class + first in the list.
