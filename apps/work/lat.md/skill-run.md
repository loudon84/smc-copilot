...
[[src/main/skill-run/skill-run-contract-parser.ts#parseSkillRunEvent]] produces Work-owned [[src/shared/skill-run.ts#SkillRunActivityItem]] items on [[src/shared/skill-run.ts#SkillRunProjection]]. [[src/main/skill-run/skill-run-service.ts#createSkillRunService]] appends a bounded list (cap 32, deduped by contract `event_id`) over the existing projection subscribe path. Native Chat rows own that activity history. [[src/renderer/src/modules/skill-run/SkillRunStatusBar.tsx#SkillRunStatusBar]] keeps phase/error, Allow/Deny, cancel, and artifact retry, and scans the same activities only for the current unread `approval.requested`.
...
M6i makes Skill execution a first-class Chat turn with Native `ChatMessage` rows, bounded live Projection, and Session-owned complete sanitized activity on reopen.
...
[[src/renderer/src/screens/Chat/Chat.tsx#Chat]] inserts the full user Prompt and a pending Native assistant row before awaiting Main `skillRun.start`. Duplicate submit is keyed by `clientRequestId` user identity. Reject or throw patches that same assistant; toast stays supplemental. [[src/renderer/src/modules/skill-run/store.ts#listSkillRunProjectionsForSession]] returns every Session projection in createdAt/request order. [[src/renderer/src/modules/skill-run/skill-run-transcript.ts#applySkillRunProjectionsToMessages]] maps sanitized prompt/activity/result onto existing Native rows. Live, reopen, and restart Skill Run history use those Native rows only; the Skill Run card union is gone. Approval still calls existing `skillRun.decideApproval`; Clarify is a read-only bubble, not Local Chat `ClarifyMessage`; Artifacts use existing File Platform preview callbacks.
...
- Local Chat / Hermes token streaming and raw Provider event transport
- Clarify respond (ClarifyCard stays read-only)
