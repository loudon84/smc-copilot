...
[[src/renderer/src/screens/Chat/Chat.tsx#Chat]] inserts the full user Prompt and a pending Native assistant row before awaiting Main `skillRun.start`. Duplicate submit is keyed by `clientRequestId` user identity, not `kind === "skill_run"`. Reject or throw patches that same assistant; toast stays supplemental. [[src/renderer/src/modules/skill-run/store.ts#listSkillRunProjectionsForSession]] returns every Session projection in createdAt/request order. [[src/renderer/src/modules/skill-run/skill-run-transcript.ts#applySkillRunProjectionsToMessages]] maps sanitized prompt/activity/result onto existing Native rows. [[src/renderer/src/modules/skill-run/SkillRunTranscriptCard.tsx#SkillRunTranscriptCard]] remains only for incomplete/rollback history until RM-04. Approval still calls existing `skillRun.decideApproval`; Clarify is a read-only bubble, not Local Chat `ClarifyMessage`; Artifacts use existing File Platform preview callbacks.
