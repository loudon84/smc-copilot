...
Wallets provisioned by the SMC Copilot backend for a profile's linked cloud agent are fetched and shown read-only alongside local ones, so the desktop stops minting wallets itself.
