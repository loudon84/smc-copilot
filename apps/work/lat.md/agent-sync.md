...
Syncs desktop profiles (the app's agents) with the signed-in [[hermes-account-login|SMC Copilot account]]'s cloud agents, bidirectionally, via the backend's `/api/agents` CRUD.
