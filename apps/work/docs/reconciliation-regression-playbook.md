...
This playbook is the repeatable gate for the sandboxed SMC Copilot reconciliation work.
Run it after SMC Copilot changes, Hermes Agent engine updates, compatibility
addon changes, or remote/SSH lab changes.

## Scope

The goal is to verify that SMC Copilot uses the dashboard event stream as the
...
Start the sandboxed SMC Copilot instance:
...
- SMC Copilot title bar says `SMC Copilot`.
...
- Gemini failures in the current lab have been traced to Hermes Agent upstream
  behavior, not SMC Copilot dashboard reconciliation.
- Plain Remote HTTP cannot be patched by SMC Copilot unless the target exposes a
...
- Legacy fallback is checked at least once after any change that touches legacy
  IPC or `/v1` paths.
- Any known failure is classified as SMC Copilot, Hermes Agent upstream, lab
