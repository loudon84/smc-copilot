...
SMC Copilot will connect directly to OAuth-gated remote Hermes dashboards through browser authentication, cookie-authenticated REST requests, and fresh single-use WebSocket tickets.

The feature applies only to direct Remote mode. Local and SSH connection behavior remains unchanged.

## User experience

SMC Copilot automatically detects the remote gateway authentication mode from the public `GET /api/status` response.
...
Given a remote URL whose `/api/status` reports `auth_required: true`, SMC Copilot identifies OAuth mode and offers browser sign-in without requesting a session token.
...
After session expiry, SMC Copilot reports that reauthentication is required and does not expose credentials, start a local gateway, or route chat through another backend.
