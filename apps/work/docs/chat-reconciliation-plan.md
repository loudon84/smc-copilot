...
existing SMC Copilot reconciliation layer. That path is useful as background, but
...
and the current SMC Copilot implementation still share the same risky shape:
...
- `src/renderer/src/screens/Chat/dashboardEventAdapter.ts` reduces ordered
  dashboard events into SMC Copilot's existing `ChatMessage[]` shape.
...
- runtime session ID and stored session ID are separated internally, but the
  surrounding SMC Copilot navigation/history model still needs a fuller stored
  session handoff;
...
Immediate SMC Copilot adjustments from this refresh:
...
- Remote/SSH pasted attachments should move from SMC Copilot staging/fallback
...
- Connection tests should probe `/api/ws` with the same credential mode the
  renderer will use.
- We should add a SMC Copilot test matching upstream's submit recovery:
...
- use the sandboxed `SMC Copilot` app identity;
...
- Does the SMC Copilot installed Hermes Agent version always ship
  `hermes dashboard`?
...
unless it proves simpler. SMC Copilot can use an intermediate timeline and then
...
Run these in the sandboxed SMC Copilot instance, with the sandbox home and non-conflicting ports:
...
- sandbox app identity `SMC Copilot`;
...
implemented in the sandboxed SMC Copilot instance and covered by unit/integration tests. Manual live
...
SMC Copilot currently depends on Hermes Agent dashboard capabilities that may not
...
dashboard default compatible with SMC Copilot.
...
- Plain remote HTTP is still probe-only. SMC Copilot cannot safely patch it
...
- Gemini failures in the current lab were traced to Hermes Agent upstream
  behavior, not the SMC Copilot dashboard transport.
...
- SMC Copilot's bundled/local Hermes Agent install;
- remote HTTP targets when SMC Copilot is allowed to deploy an addon;
- SSH targets through the tunnel/SSH deployment path.

If the target cannot be modified, SMC Copilot should keep working in legacy mode
...
- separate SMC Copilot changes from Hermes Agent compatibility changes;
- document the compatibility contract SMC Copilot expects from Hermes Agent;
...
SMC Copilot currently merges two views of chat state:
...
## Current SMC Copilot Code Paths
...
The important part for SMC Copilot is `error` as metadata, not as normal assistant
text.
...
SMC Copilot should copy this invariant: failed turns should not trigger a broad DB
...
SMC Copilot needs an adapted version that understands `content` bubbles rather
than upstream's `parts` model.

## Scope Decision

Do not migrate SMC Copilot to upstream's `parts[]` model in this fix. That would
touch rendering, transcript copying, history mapping, and live tool rendering all
at once.

Do not migrate SMC Copilot to the upstream dashboard/gateway architecture in this
fix. That is a larger product direction decision.

Instead:

- keep SMC Copilot's current `ChatMessage` union
...
Adapt upstream's helper to SMC Copilot.
...
SMC Copilot currently can surface errors through `onError`, but failures may also
