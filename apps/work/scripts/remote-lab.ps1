...
function Set-LabComfyUiBridge {
  ...
  if (Test-Path $skillPath) {
    ...
    $marker = "## SMC Copilot Remote Lab Bridge"
...
When you create a container-local copy for display in SMC Copilot, write it
...
function Configure-Desktop {
  ...
  Write-Host "SMC Copilot sandbox desktop config now points to http://127.0.0.1:$Port in remote auto mode."
}
