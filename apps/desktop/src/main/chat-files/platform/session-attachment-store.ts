...
function findMatchingUserMessageId(
  ...
  // Only touch the legacy attachments table when skipping already-stored
  // images — and only if it exists. Fresh profiles have no
  // desktop_message_attachments yet; prepare() would throw SQLITE_ERROR.
  const hasAttachments =
    skipIfHasStoredImages && tableExists(db)
      ? db.prepare(`SELECT 1 FROM ${TABLE} WHERE message_id = ? LIMIT 1`)
      : null;
