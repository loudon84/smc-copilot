...
import { PromptNavigator } from "./prompt-navigator/PromptNavigator";
import { buildPromptNavigationItems } from "./prompt-navigator/promptNavigatorUtils";
import { usePromptNavigator } from "./prompt-navigator/usePromptNavigator";
...
function Chat({
  ...
  const [promptNavigatorCompact, setPromptNavigatorCompact] = useState(false);
  const {
  ...
  const handleOpenDocumentPreview = useCallback(
    (input: Parameters<typeof openDocumentPreview>[0]) => {
      closePreview();
      openDocumentPreview(input);
    },
    [closePreview, openDocumentPreview],
  );

  const handleCloseAnyPreview = useCallback(() => {
    closePreview();
    closeDocumentPreview();
  }, [closePreview, closeDocumentPreview]);

  const activePreviewState: FilePreviewState | null = documentPreviewState.open
    ? {
        open: true,
        loading: false,
        messageDocument: {
          sessionId: documentPreviewState.sessionId || "",
          messageId: documentPreviewState.messageId || "",
          title: documentPreviewState.title || "Preview",
          content: documentPreviewState.content || "",
        },
        descriptor: {
          fileId: "",
          type: "markdown",
  ...
  const [filePreviewMaximized, setFilePreviewMaximized] =
    useFilePreviewMaximized(!!activePreviewState, active);
  ...
  const { containerRef, bottomRef, scrollToNode } = useChatScroll(messages);

  const promptNavigationItems = useMemo(
    () =>
      buildPromptNavigationItems(messages, {
        attachmentFallback: (count) =>
          t("chat.promptNavigator.attachmentFallback", { count }),
        emptyFallback: t("chat.promptNavigator.emptyFallback"),
      }),
    [messages, t],
  );

  const { activePromptId, jumpToPrompt } = usePromptNavigator({
    runId,
    active,
    containerRef,
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(([entry]) => {
      setPromptNavigatorCompact(entry.contentRect.width < 900);
    });
  ...
  return (
    ...
    >
      ...
      <FileServiceUnavailableBanner profile={profile} />

      <div
        className={[
          "chat-body",
          filePreviewMaximized ? "chat-body-preview-maximized" : "",
        ]
          .filter(Boolean)
          .join(" ")}
      >
            ...
            {messages.length === 0 ? (
              ...
              <MessageList
                ...
                onDocumentFileCreated={(fileId) => {
                  ...
                  closeDocumentPreview();
                  handleOpenManagedPreview(fileId);
                }}
                profile={profile}
        ...
        {hermesSessionId && sessionFilesVisible && (
          <SessionFilesPanel
            ...
            onHide={() => setSessionFilesVisible(false)}
          />
        )}
        ...
        {activePreviewState && (
          <FilePreviewPanel
            ...
            onToggleMaximized={() =>
              setFilePreviewMaximized((current) => !current)
            }
            onClose={handleCloseAnyPreview}
